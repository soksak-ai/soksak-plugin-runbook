    ...
    mount(container: HTMLElement, vctx?: unknown) {
      ...
      function renderRows(commands: Record<string, unknown>[]) {
        ...
        if (!commands.length) {
          ...
          // 정적 SVG(사용자 데이터 0 — XSS 안전). 런북=목록 글리프.
          icon.innerHTML =
            "<svg width='40' height='40' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'><rect x='4' y='3' width='16' height='18' rx='2'/><path d='M8 7.5h8M8 11.5h8M8 15.5h5'/></svg>";
